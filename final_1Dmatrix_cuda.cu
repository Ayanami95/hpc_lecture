...
__global__ void build_up_b(double u[], double v[],double p[]){

    int i = threadIdx.x + blockIdx.x * bolckDim.x;
...
__global__ void pressure_boundary(double p[]){
...
int main(void){
    ...
    double *cu_u,*cu_un,*cu_v,*cu_vn,*cu_p,*cu_pn,*cu_b;
    ...
    cudaMemcpy(cu_u,u,size,cudaMemcpyHostToDevice);
    cudaMemcpy(cu_un,un,size,cudaMemcpyHostToDevice);
    cudaMemcpy(cu_v,v,size,cudaMemcpyHostToDevice);
    cudaMemcpy(cu_vn,vn,size,cudaMemcpyHostToDevice);
    cudaMemcpy(cu_p,p,size,cudaMemcpyHostToDevice);
    cudaMemcpy(cu_pn,pn,size,cudaMemcpyHostToDevice);
    cudaMemcpy(cu_b,b,size,cudaMemcpyHostToDevice);
    ...
    double temp_u[size], temp_v[size], temp_p[size];

    for(int i=1;i<nt+1;i++){
        //cavity_flow(u,un,v,vn,p,pn,b);
        build_up_b<<<blockNumber,threadsPerBlock>>>(cu_u, cu_v, cu_b);
        cudaDeviceSynchronize();
        for(int j=1;j<nit+1;j++){
            pressure_poisson<<<blockNumber,threadsPerBlock>>>(cu_p, cu_pn, cu_b);
            pressure_boundary<<<blockNumber,threadsPerBlock>>>(cu_p);
            temp_p = cu_pn; 
            cu_pn = cu_p;
            cu_p = temp_p;
        }
        cudaDeviceSynchronize();
        cavity_flow<<<blockNumber,threadsPerBlock>>>(cu_u, cu_un, cu_v, cu_vn, cu_p, cu_pn, cu_b);
        velocity_boundary<<<blockNumber,threadsPerBlock>>>(cu_u, cu_v);
        temp_u = cu_un; 
        cu_un = cu_u; 
        cu_u = temp_u;
        temp_v = cu_vn; 
        ...
        cu_v = temp_v;
        cudaDeviceSynchronize();
        cudaMemcpy(u, cu_u, size, cudaMemcpyDeviceToHost);
        cudaMemcpy(v, cu_v, size, cudaMemcpyDeviceToHost);
        cudaMemcpy(p, cu_p, size, cudaMemcpyDeviceToHost);
    }
